    ...
    public int addStageToRace(int raceId, String stageName, String description, double length, LocalDateTime startTime, StageType type)
        ...
        switch (type){
            ...
            case StageType.HIGH_MOUNTAIN:
                ...
                break;
            case StageType.TT:
                race.addStage(this.nextStageID++,new Timetrial(type,this.rms.getRiderIds(),description,length,startTime));// dont need start time here, lacks mass start
        }
...
//    }
    ...
    public int[] getRidersMountainPointsInRace(int raceId) throws IDNotRecognisedException {
        ...
        int[] riderIds = this.rms.getRiderIds();
        ...
        for (int id : race.getStages()) {
            Stage stage = race.getStageByID(id);
            for (int i = 0; i < riderIds.length; i++) {
                // Get the rider's mountain points in the race
                mountainPoints[i] = stage.getMountainPointsMap().get(i).intValue();
            }
        }
